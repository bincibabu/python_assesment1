...
# to generate CSV file for cutomer Data,in the specified location
def generate_customers(output_location_root, number_of_customers, return_data=True):
    customers = []
    # Avoid new line from generated csv
    with open(f'{output_location_root}/customers.csv', mode='w',newline='', encoding='utf-8') as customers_file:
        ...
        # Performing the loop to get customer_id with in the range().
        for cid in range(1, number_of_customers + 1):
            # Generate random numbers for 'loyality_score'
            score = np.random.randint(low=1, high=11)
            customer_id = f"C{cid}"
            # Adding data to csv rows.
...
# To generate CSV file for Products
...
# producing the transaction for each customers using customer.csv and product.csv also defining startdate and enddate
